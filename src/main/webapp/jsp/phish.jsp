      ...
      <div class="container mx-auto p-6">
        <div class="glass p-6 rounded-xl shadow-lg">
          <div class="flex items-start justify-between gap-3">
            <div>
              <h1 class="text-2xl font-semibold">Phishing URL Check</h1>
              <p class="text-slate-200">
                Scan URLs and see why they might be risky.
              </p>
            </div>

            <a href="${pageContext.request.contextPath}/jsp/index.jsp" class="text-indigo-200 hover:text-white text-sm">
              Back home
            </a>
          </div>

          <!-- Form -->
          <form method="post" action="${pageContext.request.contextPath}/scan/url" class="mt-4 space-y-3">
            <input name="url" type="url" placeholder="https://example.com/login"
              class="w-full p-3 rounded bg-white/10 border border-white/10 focus:outline-none focus:ring-2 focus:ring-indigo-400"
              required />

            <div class="flex items-center gap-3">
              <button type="submit" class="px-4 py-2 bg-indigo-600 hover:bg-indigo-500 rounded shadow">
                Scan URL
              </button>

              <p class="text-sm text-slate-300">
                We never store URLs unless you're logged in for history.
              </p>
            </div>
          </form>
          ...
          <c:if test="${not empty requestScope.result}">
            ...
            <div class="mt-5 p-4 rounded-lg ${riskClass}">
              <h2 class="text-xl font-semibold">
                Result: ${requestScope.result.risk}
                — Score ${requestScope.result.score}
              </h2>

              <p class="mt-2">
                ${requestScope.result.explanation}
              </p>
          ...
          <div class="mt-6 text-slate-200">
            ...
            <ul class="list-disc ml-5 space-y-1">
              <li>Hover over links before clicking; verify the domain spelling.</li>
              <li>Enable browser protections and multi-factor authentication.</li>
